...
export const ResourceListItem = (props) => <li className='list-item'><a href="#target" onClick={() => props.onclick(props)}>{props.provider.name}</a></li> ;
...
export const ResourceList = (props) => {
    return(
        <ul className='child-menu' 
            id={props.category} 
            key={props.category}>
                {props.resourcesObject.filter(resource => resource.type === props.category).map((resource, i) =>
                     <ResourceListItem 
                        provider={resource}
                        key={i} 

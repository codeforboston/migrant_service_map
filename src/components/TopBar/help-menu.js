...
class Link extends Component {
  constructor(props) {
    ...
    this.handleClick = (href) => {
      if (helpWindowReference[href] == null || helpWindowReference[href].closed) {
  ...
  render() {
    return <a href={this.props.href} onClick={(e) => {this.handleClick(this.props.href); e.preventDefault()}}><h2>{this.props.title}</h2></a>
  }
}
...
class HelpMenu extends Component {
  helpLinks = [
    ...
    { title: "Update Provider Info", url: "https://forms.gle/2Ro2nWhGzfHmnx3m9" }
  ]
    ...
    return (
      <div className={this.props.className}>
        ...
        <div className="help-links">
          {this.helpLinks.map(link => <Link href={link.url} title={link.title} />)}
        </div>
      </div>
)
  }
